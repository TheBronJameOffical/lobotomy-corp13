...
/obj/item/ego_weapon/violet_curse/afterattack(atom/A, mob/living/user, proximity_flag, params)
	...
	if(do_after(user, 5, src))
		...
		user.pixel_z = 16
		to_chat(user, "<span class='warning'>You leap into the air!</span>")
		user.forceMove(target)
		user.AdjustStun(1 SECONDS, ignore_canstun = TRUE) //no moving while midair
		var/obj/effect/temp_visual/warn1x1/W = new(target)
		W.color = "#8206d8"
		sleep(1 SECONDS)
